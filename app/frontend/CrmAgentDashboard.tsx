...
type Analytics = {
  ...
  busiest_channels: Record<string, number>;
};

export default function CrmAgentDashboardTsx() {
  const [customerId, setCustomerId] = useState<string>(localStorage.getItem("crm_customer_id") || "");
  const [customer, setCustomer] = useState<Customer | null>(null);
  const [conversation, setConversation] = useState<Message[]>([]);
  const [analytics, setAnalytics] = useState<Analytics | null>(null);
  const [meta, setMeta] = useState<string>("Intent, tags, and escalation details will appear here.");
  ...
  async function useLastCustomer(): Promise<void> {
...
function TagList({data}: {data: Record<string, number>}) {
  return (
    <div>
...
const textarea: React.CSSProperties = {...field, minHeight: 88};
...
const pill: React.CSSProperties = {...button, marginTop: 0};
